...
def classify_error(error: BaseException) -> str:
    """Classify an error without treating every exception as transport."""
    if isinstance(
        error,
        (
            TimeoutError,
    ...
    # Assertion and validation failures represent an unexpected business
    # response, even when that response mentions a timeout or connection.
    if isinstance(error, (AssertionError, ValueError, KeyError)):
        return "business"
    ...
    if any(
        ...
        for marker in (
            ...
            "remote host closed",
            "broken pipe",
            "signalr closed the connection",
            "timed out waiting for signalr",
        )
    ):
        return "transport"
...
class StressSummary:
    ...
    def unexpected_responses(self) -> int:
        return sum(
            not sample.passed and sample.error_category != "transport"
    ...
    def transport_errors(self) -> int:
        return sum(
            sample.error_category == "transport"
            for sample in self.samples
        )
