...
def run_eps73_negative_flow(
    ...
    active_cases = cases or build_eps73_negative_cases(run_settings)
    ...
    if selected != "all":
        ...
        if not active_cases:
            available = ", ".join(
                case.case_id for case in cases or build_eps73_negative_cases()
            )
            raise ValueError(
    ...
    report.register(
        ...
        "5. [EPS-73] Setup RegisterInbound - TC-05",
        "6. [EPS-73] Setup destination.assign - TC-05",
        "7. [EPS-73] Setup bag.close - TC-05",
        "8. [EPS-73] destination.assign after bag.close - TC-05",
    )
