...
class DestinationAssignmentSuccessResult:
...
def success_step_names(start_step: int = 6) -> tuple[str, ...]:
    return (
        f"{start_step}. [Destination Assignment] RegisterItem - TC-01 with chute",
        f"{start_step + 1}. [Destination Assignment] route.assign - TC-01 with chute",
        f"{start_step + 2}. [Destination Assignment] RegisterItem - TC-02 without chute",
        f"{start_step + 3}. [Destination Assignment] route.assign - TC-02 without chute",
    )
...
def run_destination_assignment_success_cases(
    ...
    """Run the two positive Destination Assignment cases inside an existing success flow."""
    (
    ...
    register_with_chute_response = run_step(
        ...
        success_message="مرسولهٔ Destination Assignment برای تخصیص با شوتر ثبت شد.",
    )
    ...
    assign_with_chute_response = run_step(
        ...
        success_message="Destination Assignment TC-01 با تخصیص مقصد و شوتر موفق شد.",
    )
    wait_between_steps(run_settings.api_delay_seconds)
    ...
    register_without_chute_response = run_step(
        ...
        success_message="مرسولهٔ Destination Assignment برای تخصیص بدون شوتر ثبت شد.",
    )
    wait_between_steps(run_settings.api_delay_seconds)
    ...
    assign_without_chute_response = run_step(
        ...
        success_message="Destination Assignment TC-02 با تخصیص مقصد بدون شوتر موفق شد.",
    )
    return DestinationAssignmentSuccessResult(
...
def _register_inbound(
    ...
    assert_success_response(response, "Destination Assignment RegisterItem")
    return response
