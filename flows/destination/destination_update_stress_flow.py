...
@dataclass
class DestinationUpdateStressResult:
    warmup_response: dict[str, Any]
    samples: list[StressSample]
    flow_name: str = "Destination Update stress"
...
def _register(
    ...
    assert_success_response(response, "Destination Update stress RegisterItem")
    return response
...
def _assign_initial(
    ...
    assert_destination_assignment_success(
        response,
        "Destination Update stress initial route.assign",
    )
    return response
...
def _auth(device: DeviceService, run_settings: Settings) -> dict[str, Any]:
    ...
    assert_success_response(response, "Destination Update stress Auth")
    return response
...
def evaluate_race_result(
    ...
    """Validate one Destination Update TC-06 race outcome."""
    ...
    if bag_status not in (0, "0"):
        return False, f"expected container.close status=0, got {bag_status!r}"

    payload = response_payload(bag_response)
    counts = payload.get("counts")
    if not isinstance(counts, dict) or "n" not in counts:
        return False, "container.close response does not contain counts.n"
    ...
    if assignment_status in (1, "1"):
        if selected_count != 0:
            return (
                ...
                f"container.close selected {selected_count} parcel(s)",
            )
        return True, ""
    ...
    if assignment_status in (2, "2"):
        ...
        if selected_count < 1:
            return (
                False,
                "assignment was rejected after bag close, but container.close "
    ...
    return (
        False,
        "expected route.assign status=1 or status=2, "
...
def _run_iteration(
    ...
    sample = StressSample(
        iteration=iteration,
        case_name="TC-06 concurrent container.close and route.assign",
        expected_status=(
            "container.close=0 and route.assign=1 with counts.n=0 "
            "or route.assign=2 after bag close"
        ),
    )
    ...
    try:
        ...
        if errors:
            ...
            sample.error_category = str(
                errors[0].get("error_category", "business")
            )
...
def run_destination_update_stress_flow(
    run_settings: Settings = settings,
) -> DestinationUpdateStressResult:
    ...
    if run_settings.stress_workers != 1:
        raise ValueError(
            "Destination Update TC-06 requires STRESS_WORKERS=1; each iteration already "
            "runs container.close and route.assign concurrently"
        )
    ...
    result = DestinationUpdateStressResult(
