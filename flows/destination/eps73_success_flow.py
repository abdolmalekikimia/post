...
class Eps73SuccessResult:
    responses: dict[str, dict[str, Any]]


def success_step_names(start_step: int = 10) -> tuple[str, ...]:
    """Return the registered report steps for EPS-73 positive cases."""
    labels = (
        "TC-01 RegisterInbound - before destination change",
        "TC-01 destination.assign - initial destination",
        "TC-01 destination.assign - changed destination",
        "TC-01 bag.close - old destination must be empty",
        "TC-01 bag.close - new destination with new chute",
        "TC-02 RegisterInbound - before destination change",
        "TC-02 destination.assign - initial destination",
        "TC-02 destination.assign - changed destination without chute",
        "TC-02 bag.close - old chute must be empty",
        "TC-02 bag.close - new destination without chute filter",
        "TC-03 RegisterInbound - before repeated assignment",
        "TC-03 destination.assign - initial destination",
        "TC-03 destination.assign - same destination and chute",
        "TC-03 bag.close - unchanged destination and chute",
...
def _barcode(run_settings: Settings, case_number: int) -> str:
    ...
    return f"{prefix}{case_number:06d}"[-24:]
...
def _register_inbound(
    ...
    assert_success_response(response, "EPS-73 RegisterInbound")
    return response
...
def _assign(
    ...
    assert_destination_assignment_success(
        response,
        operation=f"EPS-73 destination.assign {barcode}",
    )
    return response
...
def _close_and_assert_count(
    ...
    response = device.close_bag(
        destination_center_code=destination,
        seal_number=f"SEAL-EPS73-{case_id}-{barcode[-6:]}",
    ...
    assert_bag_close_response(
        response,
        expected_status=0,
        operation=f"EPS-73 {case_id} bag.close",
    )
...
def run_eps73_success_cases(
    ...
    """Run EPS-73 positive destination update cases in one authenticated flow."""
    ...
    initial = _report_step(
        ...
        "مقصد اولیهٔ EPS-73/TC-01 تخصیص داده شد.",
    )
    step_index += 1
    _wait(wait_between_steps, run_settings)
    changed = _report_step(
        ...
        "مقصد و شوتر EPS-73/TC-01 جایگزین شد.",
    )
    step_index += 1
    _wait(wait_between_steps, run_settings)
    ...
    new_bag = _report_step(
        ...
        "مرسوله با مقصد و شوتر جدید EPS-73/TC-01 پیدا شد.",
    )
    step_index += 1
    responses["TC-01"] = {
        "register": register,
    ...
    initial = _report_step(
        ...
        "مقصد اولیهٔ EPS-73/TC-02 تخصیص داده شد.",
    )
    step_index += 1
    _wait(wait_between_steps, run_settings)
    changed = _report_step(
        ...
        "مقصد EPS-73/TC-02 بدون شوتر جدید جایگزین شد.",
    )
    step_index += 1
    _wait(wait_between_steps, run_settings)
    old_chute = _report_step(
        ...
        "مرسوله با شوتر قبلی EPS-73/TC-02 پیدا نشد.",
    )
    step_index += 1
    _wait(wait_between_steps, run_settings)
    new_bag = _report_step(
        ...
        "مرسولهٔ بدون شوتر EPS-73/TC-02 پیدا شد.",
    )
    step_index += 1
    responses["TC-02"] = {
        "register": register,
    ...
    initial = _report_step(
        ...
        "مقصد اولیهٔ EPS-73/TC-03 تخصیص داده شد.",
    )
    step_index += 1
    _wait(wait_between_steps, run_settings)
    repeated = _report_step(
        ...
        "تخصیص تکراری EPS-73/TC-03 بدون تغییر موفق شد.",
    )
    step_index += 1
    _wait(wait_between_steps, run_settings)
    bag = _report_step(
        ...
        "مقصد و شوتر EPS-73/TC-03 بدون تغییر باقی ماند.",
    )
    step_index += 1
    responses["TC-03"] = {
        "register": register,
    ...
    initial = _report_step(
        ...
        "مقصد اولیهٔ EPS-73/TC-04 تخصیص داده شد.",
    )
    step_index += 1
    _wait(wait_between_steps, run_settings)
    repeated_destination = _report_step(
        ...
        "درخواست EPS-73/TC-04 با شوتر متفاوت موفق شد.",
    )
    step_index += 1
    _wait(wait_between_steps, run_settings)
    ...
    return Eps73SuccessResult(responses=responses)
