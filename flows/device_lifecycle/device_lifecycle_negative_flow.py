...
@dataclass
class DeviceLifecycleNegativeResult:
    admin_token: str
...
def run_device_lifecycle_negative_flow(
    run_settings: Settings = settings,
) -> DeviceLifecycleNegativeResult:
    report = ExecutionReport("Device Lifecycle negative scenarios")
    report.register(
        "1. [PRECONDITION] Valid Admin Login",
        "2. [Device Lifecycle] Admin Login - invalid username",
        "3. [Device Lifecycle] Admin Login - invalid password",
        "4. [Device Lifecycle] Admin Login - empty credentials",
        "5. [Device Lifecycle] Update Device IP - invalid IP format",
        "6. [Device Lifecycle] Update Device IP - unknown device (upsert)",
        "7. [Device Lifecycle] Update Device IP - missing admin token",
        "8. [Device Lifecycle] Auth - invalid deviceId",
        "9. [Device Lifecycle] Auth - invalid deviceToken",
        "10. [Device Lifecycle] Auth - empty deviceToken",
        "11. [Device Lifecycle] WebSocket - invalid handshake protocol",
        "12. [Device Lifecycle] RegisterItem - before Auth",
        "13. [Device Lifecycle] RegisterItem - malformed payload",
        "14. [Device Lifecycle] Auth - invocation after connection close",
    )
    ...
    run_http_case(
        "6. [Device Lifecycle] Update Device IP - unknown device (upsert)",
        "unknown_device",
        # QA OPEN QUESTION (Device Lifecycle): the current Gateway-only environment
    ...
    report.print()
    return DeviceLifecycleNegativeResult(
...
def _run_invalid_handshake_case(
    ...
    try:
        return _negative_step(
            report=report,
            step_name="11. [Device Lifecycle] WebSocket - invalid handshake protocol",
            action=lambda: _connect_and_run(
                ws,
                run_settings,
