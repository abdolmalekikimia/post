...
@dataclass
class LazyUploadNegativeResult:
...
def _valid_image(**overrides: Any) -> dict[str, Any]:
    image = {
        "imageId": "lazy_upload-negative-image",
...
def build_lazy_upload_negative_cases(
    run_settings: Settings = settings,
) -> tuple[LazyUploadNegativeCase, ...]:
    return (
...
def run_lazy_upload_negative_flow(
    run_settings: Settings = settings,
    cases: tuple[LazyUploadNegativeCase, ...] | None = None,
) -> LazyUploadNegativeResult:
    active_cases = (
        cases
    ...
    if selected_case != "all":
        ...
        if not active_cases:
            ...
            raise ValueError(
                f"Unknown Lazy Upload negative case {selected_case!r}. "
                f"Available cases: {available}"
            )

    report = ExecutionReport("Lazy Upload negative scenarios")
    report.register(
        ...
        *(
            f"{index + 5}. [Lazy Upload] RegisterItem - {case.name}"
    ...
    try:
        ...
        for index, case in enumerate(active_cases, start=5):
            _wait(run_settings)
            step_name = f"{index}. [Lazy Upload] RegisterItem - {case.name}"

            def register_case(
                case: LazyUploadNegativeCase = case,
            ) -> dict[str, Any]:
            ...
            responses[case.name] = run_step(
                ...
                success_message=f"سناریوی Negative Lazy Upload/{case.name} نتیجهٔ مورد انتظار را داد.",
            )
    finally:
        ws.close()

    report.print()
    return LazyUploadNegativeResult(responses=responses, report=report)
