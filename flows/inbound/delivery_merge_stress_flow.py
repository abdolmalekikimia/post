...
from flows.inbound.history_backend_stress_flow import barcode24
...
def build_delivery_merge_stress_cases(
    ...
    if run_settings.delivery_merge_stress_fixtures_ready:
        return cases
    return tuple(case for case in cases if not case.fixture_required)


def run_delivery_merge_stress_flow(
    run_settings: Settings = settings,
) -> InboundStressResult:
    return run_inbound_stress_flow(
        run_settings,
        build_delivery_merge_stress_cases(run_settings),
        "Delivery Merge stress",
    )
