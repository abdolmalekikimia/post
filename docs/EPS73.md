...
موارد مثبت آن داخل Smoke Flowها و در Success Flow مستقل task-oriented اجرا
می‌شوند و سناریوی منفی task-oriented در مسیر Negative قرار دارد.
...
مرسوله ابتدا به یک مرکز معتبر مانند `59544` و شوتر `CH-A` تخصیص می‌یابد،
سپس به مرکز معتبر دیگری مانند `11369` و شوتر `CH-B` تغییر می‌کند. با
`bag.close` مقصد قدیمی باید
`counts.n=0` و مقصد جدید با فیلتر `CH-B` باید حداقل یک مرسوله نشان دهد.

### TC-02 — تغییر مقصد بدون `chuteId`

مرسوله ابتدا با شوتر `CH-A` ثبت می‌شود و سپس مقصد به یک کد معتبر دیگر مانند
`11369` تغییر می‌کند،
...
مرسوله به مقصد معتبر `59544` و شوتر `CH-A` تخصیص داده می‌شود، سپس `bag.close`
برای همان مقصد اجرا می‌شود. بعد از آن تغییر مقصد به کد معتبر `71956` با شوتر `CH-B` ارسال
می‌شود.
