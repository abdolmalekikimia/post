...
def assert_status_override_status(
    ...
    """Validate the Upstream status override contract for Status Override."""
    ...
    assert payload.get("errorMessage") in (None, ""), (
        f"{operation}: expected no errorMessage for Upstream override; "
        f"response={response}"
    )
...
def assert_status_override_no_override(
    response: dict[str, Any],
    ...
    assert normalized not in (3, 4), (
        f"{operation}: unexpected Status Override override status={status!r}; "
        f"response={response}"
    )
    if expected_correlation_id is not None:
