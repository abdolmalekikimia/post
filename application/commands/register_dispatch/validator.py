...
class RegisterDispatchValidator:
    """
    Validator for RegisterDispatchCommand (separate from Handler)
    ...
    @classmethod
    def validate(cls, command: RegisterDispatchCommand) -> list[str]:
        """
        ...
        if not command.dispatch_id or not str(command.dispatch_id).strip():
            errors.append("dispatch_id: required")

        # BagBarcodes validation
        if not command.bag_barcodes:
            errors.append("bag_barcodes: required")
        elif len(command.bag_barcodes) == 0:
            errors.append("bag_barcodes: must not be empty")
        ...
        # ScheduledAtUtc validation
        if not command.scheduled_at_utc:
            errors.append("scheduled_at_utc: required")
    ...
    @classmethod
    def validate_and_raise(cls, command: RegisterDispatchCommand) -> None:
