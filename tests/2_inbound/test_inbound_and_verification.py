...
from flows.success.task_success_flows import run_eps64_success_flow
...
from flows.inbound.core_history_flow import (
    CORE_HISTORY_CASES,
    run_core_history_flow as run_eps53_success_flow,
)
...
def test_eps53_core_history_success_cases():
    _require_e2e()
    if os.getenv("RUN_SUCCESS", "0") != "1" and os.getenv("RUN_EPS53_SUCCESS", "0") != "1":
        pytest.skip("Set RUN_SUCCESS=1 or RUN_EPS53_SUCCESS=1 to run EPS-53 success scenario")
    success_cases = tuple(
        case for case in CORE_HISTORY_CASES if case.name in ("success_no_discrepancy", "success_with_discrepancy")
    )
    result = run_eps53_success_flow(cases=success_cases)
...
def test_eps53_negative_scenarios():
    _require_e2e()
    if os.getenv("RUN_NEGATIVE", "0") != "1" and os.getenv("RUN_EPS53_NEGATIVE", "0") != "1":
        pytest.skip("Set RUN_NEGATIVE=1 or RUN_EPS53_NEGATIVE=1 to run EPS-53 negative scenarios")
...
def test_eps55_positive_orchestration_cases():
    _require_e2e()
    if os.getenv("RUN_SUCCESS", "0") != "1" and os.getenv("RUN_EPS55_SUCCESS", "0") != "1":
        pytest.skip("Set RUN_SUCCESS=1 or RUN_EPS55_SUCCESS=1 to run EPS-55 positive scenarios")
...
def test_eps55_negative_orchestration_cases():
    _require_e2e()
    if os.getenv("RUN_NEGATIVE", "0") != "1" and os.getenv("RUN_EPS55_NEGATIVE", "0") != "1":
        pytest.skip("Set RUN_NEGATIVE=1 or RUN_EPS55_NEGATIVE=1 to run EPS-55 negative scenarios")
...
def test_eps64_valid_image_registration_success():
    _require_e2e()
    if os.getenv("RUN_SUCCESS", "0") != "1" and os.getenv("RUN_EPS64_SUCCESS", "0") != "1":
        pytest.skip("Set RUN_SUCCESS=1 or RUN_EPS64_SUCCESS=1 to run EPS-64 success scenario")
...
def test_eps64_negative_scenarios():
    _require_e2e()
    if os.getenv("RUN_NEGATIVE", "0") != "1" and os.getenv("RUN_EPS64_NEGATIVE", "0") != "1":
        pytest.skip("Set RUN_NEGATIVE=1 or RUN_EPS64_NEGATIVE=1 to run EPS-64 negative scenarios")
...
def test_eps66_state_driven_positive_scenarios():
    _require_e2e()
    if os.getenv("RUN_SUCCESS", "0") != "1" and os.getenv("RUN_EPS66_SUCCESS", "0") != "1":
        pytest.skip("Set RUN_SUCCESS=1 or RUN_EPS66_SUCCESS=1 to run EPS-66 positive scenarios")
...
def test_eps66_state_driven_negative_scenarios():
    _require_e2e()
    if os.getenv("RUN_NEGATIVE", "0") != "1" and os.getenv("RUN_EPS66_NEGATIVE", "0") != "1":
        pytest.skip("Set RUN_NEGATIVE=1 or RUN_EPS66_NEGATIVE=1 to run EPS-66 negative scenarios")
    selected = os.getenv("EPS66_CASE", "all").casefold()
    cases = tuple(
        case
        for case in build_eps66_cases()
...
def test_eps68_override_scenarios():
    _require_e2e()
    if os.getenv("RUN_NEGATIVE", "0") != "1" and os.getenv("RUN_EPS68_NEGATIVE", "0") != "1":
        pytest.skip("Set RUN_NEGATIVE=1 or RUN_EPS68_NEGATIVE=1 to run EPS-68 negative scenarios")
