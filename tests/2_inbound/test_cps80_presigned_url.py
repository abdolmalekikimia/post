...
def test_cps80_presigned_url_success_scenarios():
    """
    اجرای سناریوهای موفق صدور Pre-signed URL و اعتبارسنجی قرارداد:
    ...
    (TC-02 آپلود مستقیم به Object Storage نیاز به دسترسی شبکه داخلی داکر دارد و در این محیط اجرا نمی‌شود)
    """
    _require_e2e()
    if os.getenv("RUN_SUCCESS", "0") != "1" and os.getenv("RUN_CPS80_SUCCESS", "0") != "1":
        pytest.skip("Set RUN_SUCCESS=1 or RUN_CPS80_SUCCESS=1 to run CPS-80 success scenarios")

    all_cases = build_cps80_cases()
    success_cases = tuple(
        case for case in all_cases if case.category in ("success_issue", "security_leakage", "provider_agnostic")
    )
...
def test_cps80_presigned_url_negative_scenarios():
    ...
    """
    _require_e2e()
    if os.getenv("RUN_NEGATIVE", "0") != "1" and os.getenv("RUN_CPS80_NEGATIVE", "0") != "1":
        pytest.skip("Set RUN_NEGATIVE=1 or RUN_CPS80_NEGATIVE=1 to run CPS-80 negative scenarios")
